...
fn main() {
	let (lod, root) = Binvox::read_file("data/lucy10.binvox");
	...
	let cam_loc = Point::new(-view_limit/2, view_limit/2, 0);
	let pitch = (-90.0).to_radians();
	let yaw = (120.0).to_radians();
	//let roll = (180.0).to_radians();
	let roll = (0.0).to_radians();
	
	let camera = Camera::new(cam_loc.clone(), pitch, yaw, roll);
	scene.set_camera(camera);
	//let screen = Screen::new(1920, 1080, 1920/2);
	let screen = Screen::new(800, 600, 800/2);//make the fov angle bigger to see everything
	...
	let filename = format!("./renders/xzylucy{}_{}_{}_{}[{}_{}_{}].ppm",
