...
fn main() {
	//let (lod, mut root, normals) = Binvox::read_file(format!("data/lamborghini.binvox"));
	let (lod, mut root, normals) = Binvox::read_file(format!("data/ktm_showcase.binvox"));
	
	let screen = Screen::new(1920, 1080, 1920/3);
	...
	let cam_loc = Point::new(0, -3*view_limit/4, view_limit/2);
	//let cam_loc = Point::new(0, 0, 0);
	let pitch = (-20.0).to_radians();
	...
	let filename = format!("./renders/ktm_showcase{}_{}_{}_{}[{}_{}_{}].ppm",
